...
export function ContactContent({ info }: Props) {
  const searchParams = useSearchParams();
  const serviceParam = searchParams.get("service");
  const defaultService = serviceParam ? serviceNames[serviceParam] : undefined;

  const items = [
    { label: "Email", value: info.email ?? "transdelsetups@gmail.com", href: `mailto:${info.email ?? "transdelsetups@gmail.com"}` },
    { label: "Phone", value: info.phone_1 ?? "+233 557 410 369", href: `tel:${(info.phone_1 ?? "+233557410369").replace(/\s/g, "")}` },
    { label: "Phone", value: info.phone_2 ?? "+233 538 134 778", href: `tel:${(info.phone_2 ?? "+233538134778").replace(/\s/g, "")}` },
    { label: "WhatsApp", value: "Chat with us", href: info.whatsapp ?? "https://wa.me/233557410369" },
    { label: "Location", value: info.location ?? "Tema, Ghana" },
  ];
